 ...
 *   nt     = 260 chapters  (Matthew → Revelation, books 39–65)
 *   ot     = 929 chapters  (Genesis → Malachi,  books 0–38)
 *   nt_ot  = 1189 chapters (all 66 books)
 *
 * NT count verified against public/bible-data.json (sum of chapters for
 * books[39..65] = 260). OT = 929. Total = 1189.
 */
...
export const PARALLEL_TABLE: Record<
  number,
  { totalDays: number; nt: number; ot: number }
> = {
...
export const NT_OT_ORDERS: { id: NtOtOrder; label: string; desc: string }[] = [
  {
    id: 'parallel',
    label: '新舊並行',
    desc: '每日新舊兩邊都讀；一邊完成後餘額自動填另一邊',
  },
  {
    id: 'nt_then_ot',
    label: '新約優先',
    desc: '優先讀新約；當日新約讀完，餘下章數自動填舊約',
  },
  {
    id: 'ot_then_nt',
    label: '舊約優先',
    desc: '優先讀舊約；當日舊約讀完，餘下章數自動填新約',
  },
]
 ...
 */
export function getSequentialDays(
  chaptersPerDay: number,
  order: 'nt_then_ot' | 'ot_then_nt',
): number {
...
/** NT ends at index 65 (啟示錄) */
export const NT_LAST_BOOK_INDEX = 65
 ...
 *
 *   NT plan:    startBookIndex ∈ [39, 65] (馬太 to 啟示錄)
 ...
 * @param books            full 66-book list (OT first, NT second)
 * @param startBookIndex   0-based canonical book index (0=創, 39=太, 65=啓)
