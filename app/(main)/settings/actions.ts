...
export async function updateReceiveNudges(newValue: boolean): Promise<Result> {
  if (typeof newValue !== 'boolean') {
    return { ok: false, error: 'invalid value: must be boolean' }
  }
  try {
    ...
    const { error } = await supabase
      .from('profiles')
      .update({ receive_nudges: newValue })
      .eq('id', user.id)
    if (error) return { ok: false, error: error.message }

    // Revalidate so the next render reads the new receive_nudges value
    // (e.g., the nudge dialog reads this profile column to filter recipients).
